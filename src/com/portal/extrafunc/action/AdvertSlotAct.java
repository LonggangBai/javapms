...
public class AdvertSlotAct {
    ...
    public String list() {
        return "extraFunc/advert/slot/list";
    }
    ...
    public String edit(Integer id, ModelMap model) {
        model.addAttribute("slot", this.service.findById(id));
    ...
    public String dataPageByJosn(Integer slotId, String sortname, String sortorder, Integer page, Integer pagesize, HttpServletRequest request, HttpServletResponse response, ModelMap model) {
        Site site = ContextTools.getSite(request);
        Pagination pagi = this.service.getPage(site.getId(), sortname, sortorder,
    ...
    public void deleteAdvertSlot(Integer[] ids, HttpServletRequest request, HttpServletResponse response)
            throws JSONException {
        JSONObject json = new JSONObject();
        AdvertSlot[] beans = this.service.deleteByIds(ids);
        for (AdvertSlot bean : beans) {
            log.info("delete AdvertSlot id={}", bean.getId());
            this.logService.operating(request, "delete add position", "id=" + bean.getId());
        }
